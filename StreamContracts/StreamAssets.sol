...
contract StreamAssets is NFToken, TokenMetaData, ReentrancyGuard{
    struct StreamAsset{
        uint256 id;
        string name; 
        string description; 
    ...
    mapping(string => uint256) public collectionStartTime;
    StreamAsset[] public allAssets; 
    string[] public allCollectionNames;
    uint256 public totalTokens = 0;
    uint256 public totalWeiUsed = 0;
    address payable public adminWallet;
    address payable public zomibeCashWallet;
    uint256 shareMultiplier;
    uint256 public totalPlainDonations;

    event UpdatedWallet(address indexed updatedBy, address indexed oldWallet, address indexed newWallet);
    event CollectionCreated(address indexed collectionOwner, string indexed name, string indexed collectionName, uint256 totalToMint, uint256 ethPricePerUnit);
    event LogFundsReceivedFromItemPurchase(address indexed buyer, string collectionName, string message, uint256 indexed itemId, uint256 totalEthPaid, uint256 indexed timeStamp);
    event LogFundsReceived(address indexed buyer, uint256 totalEthPaid, uint256 indexed timeStamp);
    
    constructor(address payable _adminWallet, string memory _tokenName, string memory _tokenSymbol, address payable _zomibeCashWallet, uint256 _shareMultiplier) 
        TokenMetaData(_tokenName,  _tokenSymbol) public {
            require(_adminWallet != address(0));
            require(_zomibeCashWallet != address(0));
            adminWallet = _adminWallet;
            zomibeCashWallet = _zomibeCashWallet;
            shareMultiplier = _shareMultiplier;
            totalPlainDonations = 0;
    }
    
    function createCollection(string calldata _name, string calldata _description, string calldata _collectionName, address _collectionOwner, uint256 _totalToMint, uint256 _ethTokenPrice,
    uint256 _collectionBuyStart) onlyOwner outOfLockdown external{
        ...
        collectionStartTime[_collectionName] = _collectionBuyStart;
        emit CollectionCreated(_collectionOwner, _name, _collectionName, _totalToMint, _ethTokenPrice);
    }
   
    function buyToken(string calldata _collectionName, string calldata _message) payable external outOfLockdown nonReentrant{
        ...
        require(collectionStartTime[_collectionName] >= now);
        uint256 id = _mintCollectionItem(_collectionName, msg.sender);
        ...
        emit LogFundsReceivedFromItemPurchase(msg.sender, _collectionName, _message, id, msg.value, now);
        _forwardFunds();
    }
    
    function () external payable {
        emit LogFundsReceived(msg.sender, msg.value, now);
        totalPlainDonations = totalPlainDonations.add(msg.value);
        _forwardFunds();
    }
  
    function _mintCollectionItem(string memory _collectionName, address _sender) internal returns(uint256){
        uint256 id = allAssets.length;
        uint256 nextItemNumber = collectionRunningCount[_collectionName].add(1);
        allAssets.push(StreamAsset(id, collectionItemName[_collectionName], collectionItemDescription[_collectionName], 
    ...
    function getTokenById(uint256 id) external view returns(uint256, string memory, string memory, string memory, uint256, uint256, uint256){
        StreamAsset memory asset = allAssets[id];
        return(asset.id, asset.name, asset.description, asset.collectionName, asset.itemNumber, asset.totalInExistance, asset.tokenPriceEth);
    }
    ...
    function getAllCollectionNamesLength() public view returns(uint256){
    ...
    function getAllTokenIdsForAddress(address _owner) external view returns(uint256[] memory ids){
    	...
    	for(uint256 i = 0;i<allAssets.length;i++){
    		StreamAsset memory asset = allAssets[i];
    ...
    function _forwardFunds() private {
        ...
        zomibeCashWallet.transfer(zombieShare);
        adminWallet.transfer(msg.value.sub(zombieShare));
    }
